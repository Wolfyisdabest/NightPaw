...
def mark_check_responded(target) -> None:
    setattr(target, "_nightpaw_check_responded", True)


def check_already_responded(target) -> bool:
    return bool(getattr(target, "_nightpaw_check_responded", False))


def allow_everywhere_slash():
    def decorator(target):
        target = app_commands.allowed_installs(guilds=True, users=True)(target)
        target = app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)(target)
        return target
    return decorator
...
def is_owner_slash(message: str | None = None):
    async def predicate(interaction: discord.Interaction) -> bool:
        if interaction.user.id != config.OWNER_ID:
            if message:
                mark_check_responded(interaction)
